...
class Model(object):
    def __init__(self, config, batch, word_mat=None, char_mat=None, trainable=True, opt=True):
        ...
        # self.ch: context char
        # self.qh: question char
        self.c, self.q, self.ch, self.qh, self.y1, self.y2, self.qa_id = batch.get_next()
        self.is_train = tf.get_variable(
            "is_train", shape=[], dtype=tf.bool, trainable=False)
        # word_mat 是词向量矩阵。每一个元素对应一个词向量，列表的行索引对应词的idx
        ...
        # c_mask size = (batch, c_max_len)
        self.c_len = tf.reduce_sum(tf.cast(self.c_mask, tf.int32), axis=1)
        self.q_len = tf.reduce_sum(tf.cast(self.q_mask, tf.int32), axis=1)

        if opt:
            # 优化的目的是取， 当前batch的最大值作为最大值。而不是config中的max值
    ...
    def ready(self):
        config = self.config
        # CL: char limit. d = hidden_size, dc=char_dim, dg=char_hidden
        ...
        with tf.variable_scope("emb"):
            with tf.variable_scope("char"):
                ...
                # 这里每个单词作为输入到rnn中，最后输出的大小是：N*PL ， 1， dg
